...
void get_hprob(const rbm *m, const double *V, double *Ph, const double *D, const int batch_size){
    int i;
...
void get_vprob(const rbm *m, const double *H, double *Pv, const int batch_size){
    ...
    for(i = 0; i < batch_size; i++){
        t1[i] = 1.0 / cblas_dasum(m->nvisible, Pv + i * m->nvisible, 1);
    }
    ...
    cblas_dsbmv(CblasRowMajor, CblasLower, batch_size * m->nvisible,
                0, 1, Pv, 1, t2, 1, 0, t1, 1);

    cblas_dcopy(m->nvisible * batch_size, t1, 1, Pv, 1);
}
